...
export function Dashboard({ email, onLogout }: { email: string; onLogout(): void }) {
  ...
  return (
    <AppShell current="overview" email={email} onLogout={onLogout}>
        {feedback && <p className={styles.demoNotice} role="status">{feedback}</p>}
        <header className={styles.pageHeader}>
          <div>
            ...
            <div className={styles.monthControl}>
              ...
              <input id="month-picker" type="month" value={month} onChange={onMonthChange} />
            </div>
          </div>
        ...
        {error && <p role="alert" className={styles.error}>{error} <button type="button" className={styles.textAction} onClick={() => { void refreshEnvelopes(); void refreshSummary(); void refreshIncome(); }}>Tentar novamente</button></p>}
        ...
        <div className={styles.contentGrid}>
          <section id="verbas" className={styles.panel} aria-labelledby="verbas-titulo">
            <div className={styles.panelHeader}><div><h2 id="verbas-titulo">Suas verbas</h2><p>Valores disponíveis neste mês</p></div><Link href="/verbas">Ver todas →</Link></div>
            {envelopes.length === 0 && !loading && !error ? (
              <div className={styles.emptyState}>
                <p>Nenhuma verba neste mês — crie a primeira para começar a reservar.</p>
          ...
          <section id="historico" className={styles.panel} aria-labelledby="historico-titulo">
            <div className={styles.panelHeader}><div><h2 id="historico-titulo">Histórico de renda</h2><p>Alterações reais, mais recente primeiro</p></div><button type="button" className={styles.textAction} onClick={() => void refreshHistory()}>Atualizar</button></div>
            <IncomeHistory
              data={history}
            ...
            <div className={styles.panelFooter}>
              <button type="button" className={styles.secondaryAction} onClick={() => setShowNewEnvelope(true)}>Nova verba</button>
              <p>
